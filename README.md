...
- Ops tooling: `musetalk-mlx-benchmark` (FPS + per-stage budgets, PRD 9.5) and `musetalk-mlx-stress` (long-session leak <= 50MB + fragmentation probe). Allocator-cache drift passes; the active-memory drift root cause is open (escalated to fusion-mlx#954) — see `tests/integration/INTEGRATION_PENDING.md`.
...
- `musetalk-mlx-stress` reports RSS + MLX active/cache/peak memory per sample; wire its JSON output into a 2h leak budget alert (threshold 50MB drift). Note: `scripts/leak_gate.py` fails on the active signal until the retention root cause (investigation at fusion-mlx#954) is resolved; the allocator-cache signal passes.
